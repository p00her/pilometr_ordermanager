...
const cmds = [
  ...
  'docker rm -f order-manager && docker run -d --restart=unless-stopped --name order-manager -p 8088:8088 -v order-manager-data:/app/data order-manager && docker network connect npm_default order-manager',
  'echo "---done---"',
];
let idx = 0;
function run() {
